...
Auk has a built-in GraphQL platform that can be used to execute GraphQL queries. The GraphQL platform can be accessed by clicking on the `GraphQL` tab in the left sidebar.
...
Auk's GraphQL API platform provides you with the best experience to test and play around with GraphQL.
...
GraphQL queries can be executed using a GraphQL client. Auk has a built-in GraphQL client that can be used to execute GraphQL queries.
