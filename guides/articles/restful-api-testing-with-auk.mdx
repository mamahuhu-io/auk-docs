...
title: RESTful API testing with Auk
description: Learn how to use Auk to test your RESTful APIs.
---

Representational State Transfer (REST) APIs are a core part of communication between a web/mobile client and a server. The majority of web-based applications depend on REST APIs to fetch and modify data thus separating the data processing part from the front-end. Auk provides a fast and intuitive platform to develop and test REST APIs making it easier for developers worldwide to work efficiently.
...
REST APIs also support HTTP methods to do operations on data. Auk supports the following methods out of the box.
...
## Using Auk to test your REST APIs

Auk provides you with a minimal yet robust platform to test your REST APIs. The REST platform is the default platform you see when you open Auk.
...
If you are planning to use Auk to test your locally served APIs, it is recommended that you install the Auk Browser Extension. Once installed, switch the interceptor on Auk to Browser Extension from the settings page or bottom bar to add support for [localhost](http://localhost) protocols.
...
Auk has support for various types of authentication mechanisms such as Basic Auth, Bearer Token, OAuth 2.0, and API Key. You can configure this using the Authorization tab on the request section of Auk.

These are just a few of the amazing set of features that Auk provides you to make your life easier as a developer. In addition to this, Auk supports features such as collections to save your requests, pre-requests to add dynamic behavior to your requests and so much more.
